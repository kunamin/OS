...
void argv2stack(void** esp, char** argv, int argc) {
  ...
  for(idx = argc - 1; idx >= 0; idx--){
    int len;
    // 지금 여기 바로 밑에서 argv[idx] 점근하면 multi-oom이란 프로세스가 한번 스택에 넣고 그다음 child가 들어오는데 이 child가 exit 되서 인지 다음줄 부터 진행이 안됨.
    len = strlen(argv[idx]) + 1;  // including '\0'
    word_align += len;
    //printf("@argv2stack - argv[%d]: %s\n", idx, argv[idx]);
    //printf("@argv2stack - esp: %s\n", esp);  // Not worth
...
<<<<<<< HEAD
    sema_down(&(child->child_not_exit));
    exit_return = child->success_exit;
    //list_remove(&(child->hanging_child));
    sema_up(&child->child_not_give_to_succ_ex);
    list_remove(&(child->hanging_child));
=======
