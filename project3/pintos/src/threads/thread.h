...
struct thread
  {
   ...
   bool user2kernel;  // true: kernel mode and false: user_mode
   uint32_t* user2kernel_esp;  // esp value at user_mode

   mapid_t mapid;

   // 230603 MKH
   // 차일드가 종료 될때 unmap 되기 전에 부모 함수가 확인할때 기다리기 위해 만듬
   // child는 부모가 unmap된 것을 확인하기 전까지 exit 되면 안된다.
   // Testcase, mmap-exit
   // Parent Process가 Read하는 것과 Child Process가 Unmap하는 것에 Synchronization을 맞추지 않으면 테스트 케이스가 통과 X
   // Child Process가 unmap하고 Parent Process가 Read할 수 있도록 semaphore를 추가
   struct semaphore child_not_give_to_unmap;
#endif
