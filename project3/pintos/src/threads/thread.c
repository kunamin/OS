               ...
               thread_func *function, void *aux) 
{
  ...
  // 새로운 thread를 create할 때 thread_create를 호출
  // 이때 thread_current는 새로운 thread를 생성하는 thread이고
  // struct thread* t는 새롭게 만들어지는 thread
  t->parent = thread_current();
  list_push_back(&thread_current()->hanging_children, &t->hanging_child);
...
thread_exit (void) 
{
...
#ifdef USERPROG
  // exiting process should release all files assigned to it's own file descriptor table
  struct thread* t = thread_current();

  // Find support_page created by map_file
  struct list_elem* curr = NULL;
  struct list_elem* next = NULL;
  // Error at below list_next()  -> Move this unmap implemenation to sys_exit 부분에서 옮기니까 잘 됨
  ...
  //   if(temp->mapid > 0) external_unmap(temp->mapid);
  // }

  // Parent process에게 child process(자신)이 unmap을 완료했음을 알림
  // write-bad-ptr에서 infinite wait하는 문제 발생
  // Move sema_up(&t->child_not_give_to_unmap) to here
  // sema_up이 external_close보다 아래에 있을 경우에 external_close->sys_close 과정에서
  // sys_exit(-1)이 될 경우에 sema_up을 시켜주지 못하고 exit(-1)이 되어서 parent process가 무한대기하게 됨
  //sema_up(&t->child_not_give_to_unmap);
  // free all the support page
  // for(curr = list_begin(&t->support_page_table); curr != list_end(&t->support_page_table); curr = next){
  //   support_page* temp = list_entry(curr, support_page, support_page_elem);
  //   next = list_next(curr);
  ...
  for (idx = 0; idx < FD_MAX; idx++){
    ...
    if(t->fd_table[idx] != NULL){
      external_close(idx);  // Use external_close (wrapper function of sys_close)
    }
  }
  ...
  //list_remove(&thread_current()->hanging_child);
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  // 230518 HJ
  // thread_init에서 옮김
  // thread_init에서만 초기화했으면 initial_thread(init.c의 main돌리는 thread)에서만 초기화되고
  // thread_create가 만드는 thread에 대해서는 초기화가 되지 않음 (thread_create가 thread_init가 아닌 init_thread를 호출하기 떄문!)
  // 그래서 여기서 초기화하는게 맞는듯?
  ...
  t->success_exit = 0;
  t->parent = NULL;
  list_init(&t->hanging_children);
  sema_init(&t->child_not_exit, 0);
  sema_init(&t->child_not_load, 0);
  sema_init(&t->child_not_give_to_succ_ex, 0);

  /* project 3 */
  list_init(&t->support_page_table);

  t->user2kernel = false;
  t->user2kernel_esp = NULL;

  t->mapid = 0;
  sema_init(&t->child_not_give_to_unmap, 0);
}
