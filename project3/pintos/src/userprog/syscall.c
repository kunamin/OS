...
// project 3
#include "vm/page.h"  // support_page at static mapid_t map_file2virtualmemory(int fd, void* addr); 
#include "vm/frame.h"
#include "userprog/exception.h"
// Additional typedef
typedef int pid_t;    // static int wait_pid(pid_t pid); and static pid_t exec_command(const char* command);
//typedef int mapid_t;  // static mapid_t map_file2virtualmemory(int fd, void* addr); and static void unmap_mappedfile(mapid_t mapped);
...
/* project 3 */
static mapid_t map_file2vm(int fd, void* addr);
static void unmap_mappedfile(mapid_t mapid);
...
syscall_handler (struct intr_frame *f UNUSED) 
{
  ...
  */
  ...
  // store esp from the user mode to use it at page_fault() which is in the kernel mode
  // interrupt에 의해 syscall이 호출됐을 때의 f->esp는 user_mode의 esp이지만
  // syscall에서 page_fault가 일어났을 때, page_fault에서의 interrupt_frame은 user_mode에서의 interrupt가 아니라서
  // f->esp가 user_mode의 esp가 아니라 kernel mode의 esp이다
  // 그래서 user_mode의 esp를 syscall에서 page_fault가 일어났을 때도 사용하기 위해 thread에 변수를 새로 선언하고 저장해준다 (idea from Stanford Manual)
  struct thread* t = thread_current();
  t->user2kernel = true;
  t->user2kernel_esp = esp;
  ...
  switch(sysnum) {                          /* Comment convention: enum. (# of arguments) functionality */
    ...
    case SYS_CLOSE:                         /* 12. (1) Close a file. */
      ...
      break;
    case SYS_MMAP:                          /* 13 (2) Map a file into memory. */
      access_addr2usermem(esp + 4);  
      access_addr2usermem(esp + 4 + 1);

      // function call                      // Will be implemented later
      f->eax = map_file2vm(*(esp + 4), *(esp + 4 + 1));
      break;
    case SYS_MUNMAP:                        /* 14 (1) Remove a memory mapping. */
      access_addr2usermem(esp + 1);

      // function call                      // Will be implemented later
      unmap_mappedfile(*(esp + 1));
      break;
  }
  // After syscall_handler, the kernel resume the code in the user mode => need to set t->user2kernel = false
  t->user2kernel = false;
}
...
void sys_exit(int status){
  ...
  // // Find support_page created by map_file
  ...
  for(curr = list_begin(&t->support_page_table); curr != list_end(&t->support_page_table); curr = next){
    ...
    next = list_next(curr);
    //printf("%d@thread_exit when%s\n", temp->mapid, thread_current()->name);
    if(temp->mapid > 0) external_unmap(temp->mapid);  // unmap으로 삭제
    //else if(temp->mapped_file != NULL && temp->load){
      //printf("hear\n");
      //update_dirty_page(&t->pagedir, temp);
    //}
    else support_page_free(temp);
  }
  ...
  //   next = list_next(curr);
  // }  
  sema_up(&t->child_not_give_to_unmap);  // child process가 unmap을 완료할 때까지 ㄱㄷ)
...
static int read_fd2buffer(int fd, void* buffer, unsigned size){
  ...
  void *buffer_copy;
  sema_down(&using_at_once);
	for (buffer_copy = buffer; buffer_copy <= buffer + size; buffer_copy += PGSIZE) {
    if(!check_addr2usermem(buffer_copy)) sys_exit(-1);
		support_page *sp = support_page_get((uint32_t)buffer_copy & ~PGMASK);
    //printf("sp load @ read: %x:%x is %d\n", sp, sp->page_num, sp->load);
    int i = 1000000;
    while(i != 0) i--;
    if(sp != NULL && sp->load == true) sp->using = true;  // Typo
    //ASSERT(sp->load == false);
      //ASSERT(false); //page-merge-par이랑 page-merge-stk일 때 여기 통과
         //bool success = support_page_load(sp);
         //sp->load = success;  // support_page를 load하는 데 성공했는지에 따라서 sp->load를 update
      //printf("sp load: %x is %d\n", sp, sp->load);
      // //   //if(success) return;
      //}
    //}
	}
  sema_up(&using_at_once);
...
static int write_buffer2fd(int fd, const void* buffer, unsigned size){
  ...
  void *buffer_copy;
    ...
    int i = 1000000;
    while(i != 0) i--;
    //printf("sp load @ write: %x:%x is %d\n", sp, sp->page_num, sp->load);  // sp load @ write: c010524c:804c000 is 1
    if(sp != NULL && sp->load == true) sp->using = true;  // Typo
    if(sp->load == false){
      //ASSERT(false); //page-merge-par이랑 page-merge-stk일 때 여기 통과
         //bool success = support_page_load(sp);
  ...
  if(fd == 1){
    ...
    */
    ...
    sema_down(&filesys_at_once);
    size = file_write(t->fd_table[fd], buffer, size);
    sema_up(&filesys_at_once);
    //return written_size;
  }
  sema_down(&using_at_once);
...
// map_file2virtualmemory 디버깅할 때 break 걸어야하는데 함수 이름이 너무 길어서 줄임
static mapid_t map_file2vm(int fd, void* addr){
  /* map fail handling from blackboard Manual */

  // fd is 0 or 1
  if (fd < 0 || fd == 0 || fd == 1 || fd >= FD_MAX){
    return -1;
  }

  //printf("addr: %x\n", PGMASK);
  //printf("addr: %d\n", (uint32_t)addr & PGMASK);
  //printf("addr: %x\n", (uint32_t)addr & PGMASK);
  // addr is 0(addr must be in user memory) or addr is not page-aligned
  if(!check_addr2usermem(addr) || (uint32_t)addr & PGMASK){
    //printf("check: %d\n", !check_addr2usermem(addr));
    //printf("addr: %x\n", (uint32_t)addr & PGMASK);
    return -1;
  }
  ...
  // file is not opened or file has a length of zero bytes
  // file_length: /* Returns the size of FILE in bytes. */
  sema_down(&filesys_at_once);
  int size_f = file_length(f);
  //printf("size_f: %d\n", size_f);
  sema_up(&filesys_at_once);
  ...
  // The range of pages mapped overlaps any exisitng set of mapped pages 이건 어떻게 처리하지?
  // support page table 돌면서 동일한 page_num에 대한 support_page가 이미 만들어져있을 경우(overlap)
  // return -1;
  struct list_elem* curr = NULL;
  struct list_elem* next = NULL;
  ...
  // Stanford Manual: You should use the file_reopen function to obtain a separate and independent reference to the file for each of its mappings
  // file_reopen으로 현재 thread->fd_table[fd]에 있는 file 복사하기 -> 완
  struct file* f_copy;
  ...
  // reopen으로 연 파일은 seperate and independent reference to the file이니까 현재 thread->fd_table에서 빈자리 찾아서 open해주기
  int idx;
  for (idx = 3; idx < FD_MAX; idx++){
    // If t->fd_table[i] is not closed while this thread is being exited
    if(t->fd_table[idx] == NULL){
      //printf("%d\n", idx);
      t->fd_table[idx] = f_copy;
      break;
    }
  }

  
  // unique한 identifier를 사용하기 위해서 struct thread 내부에 mapid를 선언해서 
  // map_file2vm이 실행될 때마다 +1을 해주면 unique하게 mapid를 maintain할 수 있다
  // unmap_mappedfile에서 mapid를 통해 찾는다
  // start with 1 (because t->mapid is initialized by 0)
  mapid_t mapid = t->mapid + 1;
  // offset is 0 at first
  off_t offset = 0;
  
  // 여기서 lazy loading 해야한다고 하니 support_page만 만들면 될 듯? => load_segment에서 support_page 만들었던 코드 ㄱㄱ
  // size_f can be exceed PGSIZE => load_segment에서 사용하던 logic 그대로
  while(size_f > 0){
    size_t read_bytes = size_f < PGSIZE ? size_f : PGSIZE;
    size_t zero_bytes = PGSIZE - read_bytes;

    support_page* sp = support_page_init();
    if(sp == NULL) return -1;
    support_page_set(sp, f_copy, offset, (uint32_t)addr & ~PGMASK, read_bytes, zero_bytes, true);

    // store mapid to retrieve whether this page is mapped from file or not (Will be used at unmap_mappedfile)
    sp->mapid = mapid;

    size_f -= read_bytes;
    addr += PGSIZE;  // page_num += 1
    offset += read_bytes;
  }
  return mapid;
}
...
static void unmap_mappedfile(mapid_t mapid){
  ...
  void* page_num = NULL;
  // Find support_page->mapid == mapped
  for(curr = list_begin(&t->support_page_table); curr != list_end(&t->support_page_table); curr = next){
      support_page* temp = list_entry(curr, support_page, support_page_elem);
      next = list_next(curr);
      // unmap_mappedfile()의 인자로 들어온 mapid와 support_page의 mapid가 일치하는지 
      // 일치 == map_file2vm에 의해 생성된 support_page
      if(temp->mapid == mapid){
        // //printf("%d@unmap_mappedfile\n", mapid);
        // mapped_file = temp->mapped_file;
        // page_num = temp->page_num;
        // //if(temp->load == true) printf("Loaded!\n");
        // //if(pagedir_is_dirty(t->pagedir, temp->page_num) == true) printf("dirty!\n");
        // //printf("%s\n", thread_current()->name);
        // if(pagedir_is_dirty(t->pagedir, page_num)){
          
        //   sema_down(&filesys_at_once);
        //   // Move mapped_file->pos(off_t) to temp->offset
        //   file_seek (mapped_file, temp->offset);
        //   sema_up(&filesys_at_once);

        //   sema_down(&filesys_at_once);
        //   // Write temp->page_num to mapped_file as much as temp->read_bytes
        //   file_write(mapped_file, page_num, temp->read_bytes);
        //   sema_up(&filesys_at_once);
        // }
        update_dirty_page(t->pagedir, temp);
        // Free suport_page
        // support_page_free(temp);
      }
  }

  // there is no mapped file which is found by mapid_t mapped
  //if(mapped_file == NULL) return;
  //printf("%x\n", mapped_file);

  // reopen 한 파일을 fd_table에 새롭게 추가했으므로 unmap일 때는 close해줘야한다
  int idx;
  for (idx = 3; idx < FD_MAX; idx++){
    // If t->fd_table[i] is not closed while this thread is being exited
    if(t->fd_table[idx] == mapped_file){
      //printf("%d\n", idx);
      //printf("%x\n", t->fd_table[idx]);
      sema_down(&filesys_at_once);
      file_close(mapped_file);  // Use external_close (wrapper function of sys_close)
      sema_up(&filesys_at_once);
      t->fd_table[idx] = NULL;
    }
  }
...
void external_unmap(mapid_t mapid){
  unmap_mappedfile(mapid);
}

void update_dirty_page(uint32_t* pagedir, support_page* sp){
    struct file* mapped_file = sp->mapped_file;
    void* page_num = sp->page_num;
    //if(temp->load == true) printf("Loaded!\n");
    //if(pagedir_is_dirty(t->pagedir, temp->page_num) == true) printf("dirty!\n");
    //printf("%s\n", thread_current()->name);
    access_addr2usermem(page_num);
    if(pagedir_is_dirty(pagedir, page_num)){
      
      sema_down(&filesys_at_once);
      // Move mapped_file->pos(off_t) to temp->offset
      file_seek (mapped_file, sp->offset);
      sema_up(&filesys_at_once);

      sema_down(&filesys_at_once);
      // Write temp->page_num to mapped_file as much as temp->read_bytes
      file_write(mapped_file, page_num, sp->read_bytes);
      sema_up(&filesys_at_once);
    }
    support_page_free(sp);
}
